...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, load_depths=False):
    ...
    for cam in range(1, 13):
        ...
        imglists = sorted(os.listdir(imgdir))
        imgfiles = imglists[:frame_num]
        
        imgfiles = [os.path.join(imgdir, f) for f in imgfiles if f.endswith('JPG') or f.endswith('jpg') or f.endswith('png')]
...
def load_llff_data(basedir, factor=1, width=None, height=None,
    ...
    if bds.min() < 0 and bd_factor is not None:
        ...
        import sys; sys.exit()
    sc = 1. if bd_factor is None or bds.min() == 0 else 1./(bds.min() * bd_factor)
    poses[:,:3,3] *= sc
    bds *= sc
    depths *= sc
    if spherify:
        ...
        focal_depth = mean_dz * movie_render_kwargs.get('scale_f', 1)
        print(f'reasonable focal depth: {focal_depth}')
        
        # Get radii for spiral path
        zdelta = movie_render_kwargs.get('zdelta', 0.2)
...
#             zloc = np.percentile(tt, 10, 0)[2]
        ...
        render_poses = render_path_spiral(c2w_path, up, rads, focal_depth, zdelta, zrate=zrate, rots=N_rots, N=N_views)
    ...
    return images, depths, poses, bds, render_poses, i_test, focal_depth, times