...
from lib import utils,dlfgo_model_mlp
...
def config_parser():
    ...
    parser.add_argument("--mlp_depth", type=int, default=8, help='color voxel grid depth')
    parser.add_argument("--mlp_width", type=int, default=256, help='color voxel grid width')
    ...
    parser.add_argument("--lr_mlp", type=float, default=5e-04, help='lr of the mlp')
...
@torch.no_grad()
def warmup(model, ray_type, H, W, K, c2w, focal_depth, time_scalar=None):
...
def render_viewpoints(model, ray_type, render_set, render_poses, HW, Ks,
    ...
    # model.eval()
    warmup(model, ray_type, H, W, K, render_poses[0], focal_depth,
...
def create_new_model(args, ray_min, ray_max):
    ...
    if args.ray_type == 'twoplane':        
        model = dlfgo_model_mlp.DLFGO_twoplane_mlp(
            ray_type='twoplane', mlp_depth=args.mlp_depth, mlp_width=args.mlp_width,
    ...
    params.append({'params': model.mlp_layers.parameters(), 'lr': args.lr_mlp})
...
def scene_rep_reconstruction(args, data_dict):
    ...
    if reload_ckpt_path is None:
        ...
        ray_min, ray_max = utils.get_ray_min_max(
            ...
            train_poses=poses[i_train], HW=HW[i_train], Ks=Ks[i_train], focal=focal_depth)
    ...
    def gather_training_rays(ray_type):
        ...
        rgb_val, ray_val = utils.get_training_rays(
            ...
            train_poses=poses[i_val], HW=HW[i_val], Ks=Ks[i_val], focal=focal_depth)
        rgb_train, ray_train = utils.get_training_rays(
            ray_type=ray_type, rgb_original=rgb_train_original,
            train_poses=poses[i_train], HW=HW[i_train], Ks=Ks[i_train], focal=focal_depth)
        
        # ray들을 concatenate하여 전체 min-max 구하기
        ray_all = torch.cat([ray_train, ray_val], dim=0)
        ray_min = ray_all.min(dim=0, keepdim=True)[0]  # [1, feature_dim]
        ray_max = ray_all.max(dim=0, keepdim=True)[0]  # [1, feature_dim]
        
        # -1~1로 normalize
        ray_train = 2.0 * (ray_train - ray_min) / (ray_max - ray_min) - 1.0
        ray_val = 2.0 * (ray_val - ray_min) / (ray_max - ray_min) - 1.0
    ...
    for global_step in pbar:
        ...
        if global_step%(1*iter_per_epoch)==0 or (global_step%(1*iter_per_epoch)==0 and global_step < (10*iter_per_epoch)) or (global_step%2000==0 and global_step < (10*iter_per_epoch)):
            ...
            write_str += f'mlp lr / {optimizer.param_groups[0]["lr"]:.8f} / '
            ...
            testsavedir = os.path.join(args.basedir, args.expname, f'render_test')
            ...
            rgbs = render_viewpoints(
                ...
                focal_depth=data_dict['focal_depth'])
...
if __name__=='__main__':
    ...
    if args.grid_num == 0:
        if args.dataset_name == 'cam':
            args.grid_num = [4, 4, 960, 540, args.frame_num]
        else:
